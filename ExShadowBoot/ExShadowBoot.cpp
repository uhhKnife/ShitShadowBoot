...
void __cdecl main()
{
	HANDLE hPayload, hXboxRom;
	DWORD szRead;

	// Load HvxShadowBoot.bin payload
	hPayload = CreateFile("GAME:\\HvxShadowBoot.bin", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hPayload == INVALID_HANDLE_VALUE)
	{
		DbgPrint("Failed to open HvxShadowBoot.bin\n");
		return;
	}
	szPayload = GetFileSize(hPayload, NULL);
	pPayload = XPhysicalAlloc(szPayload, MAXULONG_PTR, 0x10000, PAGE_READWRITE | MEM_LARGE_PAGES);
	if (pPayload == NULL)
	{
		DbgPrint("Failed to allocate HvxShadowBoot.bin\n");
		return;
	}
	if (ReadFile(hPayload, pPayload, szPayload, &szRead, NULL) == FALSE || szRead < szPayload)
	{
		DbgPrint("Failed to read HvxShadowBoot.bin\n");
		return;
	}
	DbgPrint("Loaded HvxShadowBoot.bin at %#x size %#x\n", MmGetPhysicalAddress(pPayload), szPayload);
